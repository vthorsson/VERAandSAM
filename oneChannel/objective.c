...
/********************************************************************************************/
/**                                                                                        **/
/**  Z_OC.c - Objective function (variables beta)                                          **/
/**  One channel version                                                                   **/
/**                                                                                        **/
/**  April 2000 Copyright (c) Vesteinn Thorsson                                            **/
/********************************************************************************************/

/* X must be shifted my muX */

double Z_OC( double *beta ){
  ...
  for (i=0 ; i<N ; i++ ){ /* loop over genes */
    ...
    /* construct C  */    
    C = Vex* muX[i]*muX[i] + Vdx;
 
    logsum += m[i] * log(C); /* one contribution from each sample */

    /* construct B, the inverse of C */
    B = ((double)1.)/C;
    for (j=0 ; j<m[i]; j++ ){ /* loop over repeats */
      quadsum += B * X[i][j] * X[i][j] ; 
    }
...
/********************************************************************************************/
/**                                                                                        **/
/**  gradZ_OC.c - Gradient of objective function (variables beta)                          **/
/**  One channel version                                                                   **/
/**                                                                                        **/
/**  April 2000 Copyright (c) Vesteinn Thorsson and Trey Ideker                            **/
/********************************************************************************************/
...
void gradZ_OC( double *beta, double *grad ){


  int i,j;
  double Sex, Sey, Vex, Vdx; 
  double temp, x, mu_x; 
  double dC_dg1, dC_dg2, multiplier;
  ...
  for (i=0 ; i<N ; i++ ){ /* loop over genes */
    ...
    multiplier = m[i]/C - temp/C/C;
    grad[1] += multiplier * dC_dg1;
    grad[2] += multiplier * dC_dg2;
...
/********************************************************************************************/
/**                                                                                        **/
/**  Z_gene_OC.c - Single gene objective function, One channel (variable mu_x, mu_y)       **/
/**                                                                                        **/
/**  July 2004  Copyright (c) Vesteinn Thorsson                                            **/
/********************************************************************************************/

/* X must not have muX subtraction */

double Z_gene_OC( double mju ){

  int j; 
  double logsum, quadsum, x;
  double C, B; 
  ...
  C = Vex* mju * mju + Vdx;
  logsum = m[gene_index] * log(C); /* one contribution from each sample */
...
/********************************************************************************************/
/**                                                                                        **/
/**  derivativeZ_gene_OC.c - Derivativeient of single gene objective function (variables mu_x)  **/
/**                                                                                        **/
/**  July 2004  Copyright (c) Vesteinn Thorsson                                            **/
/** This function has yet to be used and tested                                            **/
/********************************************************************************************/

/* X must not have muX subtraction */

double derivativeZ_gene_OC( double mju ){
...
#ifdef MUNONNEG  
  /* take absolute values of mju but retain sign to adjust derivativeient */
  if ( mju >= 0 ){ s  = 1 ; } else { s = -1; }
  mju  =  fabs( mju  );
#endif
  ...
  derivative = dC_dg * m[gene_index] / C; 
  
  for (j=0 ; j<m[gene_index]; j++ ){ /* loop over repeats */
    
    x = X[gene_index][j] - mju;
    
    derivative -= dC_dg * x * x /C/C; 
    derivative -= 2.*B * x; 
    
  }

  derivative = derivative/((double)2.); 
  ...
  if ( mju >= 0 && mju <=  1e-8 && derivative > 0. ) derivative = 0  ;
...
static void funcd(double mju,double *fn, double *df)
{
  *fn = Z_gene_OC(mju);
  *df = derivativeZ_gene_OC(mju); 
}
