...
#define MAXGENES 70000  /* maximum no. genes for temporary storage */
...
/***************************************************************************************************/
/** read_data_OC.c                                                               **********************/
/** read single channgle data file                                              **********************/
/***************************************************************************************************/
...
#define MAXLENGTH 30 /* maximum length of gene name strings */
  ...
  int mstore[MAXGENES], internal2full_temp[MAXGENES] ;
  int Nkeep=0, mtemp=0;
  int N_col, Xcol[MAXREPS],  Fcol[MAXREPS], index_to_find;
  int file_position, line_length, nXcols, nFcols, nreps;
  int n_header_col, n_data_col; 
  char index_to_find_string[MAXREPS], full_string[MAXREPS]; 
  char yorf_word[MAXLENGTH], gene_name_word[MAXLENGTH], *token; 
  ...
  /*** Print out summary on column detection */
  printf("Maximum %d replicates, from column headings: ", nreps);
  for ( i=0 ; i<nreps ; i++ ) printf( "(%d) ", Xcol[i] ) ;
  printf("\n");
  if ( N_col != -1 ){
    printf("Taking actual number of pairs from N column (column %d)\n", N_col);
  } else { 
    ...
    if ( nFcols != -1 ){
      printf("or with an 'X' in F columns ( ");
      for ( i=0 ; i<nreps ; i++ ) printf( "%d ", Fcol[i] ) ;
      printf(")");
    }
  }
  printf("\n");
