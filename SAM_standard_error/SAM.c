...
main (int argc,char *argv[] ){
  ...
  double *hess_mu, *covariance_in_means;
  double detH; 
  double *stderrX, *stderrY, *covXY;
  ...
  stderrX = (double *) calloc(N, sizeof(double));
  stderrY = (double *) calloc(N, sizeof(double));
  covXY = (double *) calloc(N, sizeof(double));
  ...
  hess_mu = (double *) calloc(4., sizeof(double)); 
  covariance_in_means = (double *) calloc(4., sizeof(double)); 
  ...
  for(i=0; i<N; i++ ){ 
    ...
    /**** compute standard error of muX and muY from Hessian of objective function  ****/ 
    /***  covariance_in_means: entries (1,1) (2,2) (1,2) respectively ****/
    hessianZ_gene(mu_vec, hess_mu );
    detH = hess_mu[1] * hess_mu[2] - hess_mu[3] * hess_mu[3] ;
    covariance_in_means[1] =  hess_mu[2]/detH ; 
    covariance_in_means[2] =  hess_mu[1]/detH ; 
    covariance_in_means[3] =  -hess_mu[3]/detH ; 
    /*** For low X,Y values, may get negatives on diagonal ***/
    /*** Set standard errors to background error estimate ***/
    /*** May want to set up counter for these exceptions ***/
    if ( covariance_in_means[1] >= 0 ){ 
      stderrX[i] = sqrt( covariance_in_means[1] );
      covXY[i]=covariance_in_means[3];
    } else {
      stderrX[i] = cov[4];
      covXY[i]=0; 
    }
    if ( covariance_in_means[2] >= 0 ){ 
      stderrY[i] = sqrt( covariance_in_means[2] );
      covXY[i]=covariance_in_means[3];
    } else {
      stderrY[i] = cov[5];
      covXY[i]=0;
  ...
  write_llr_tabbed( fpin, fpout, loglr, stderrX, stderrY, covXY);
