...
void write_llr( FILE *fpin, FILE *fpout, double *llrvals, double *stderrX, double *stderrY, double *covXY ){
  ...
  int tempered_count=0 , std_err_warning_count=0 ; 
  float tempered_log_ratio ( float, float, float, float, char* ); 
  float log_ratio_stderr ( float, float, float, float, float, char, float, float);
  float max_threshold; 
  float stderr_lograt ; 
  int alargenumber = 9999999; 
...
#ifdef MUNONNEG
  fputs( "      mu_X       mu_Y       lambda   muRATIO  T", fpout);  
  fputs( " StandErr_X StandErr_Y    CovarXY StdErr_RAT\n", fpout);  
#else
  fputs( "      mu_X       mu_Y       lambda", fpout);  
  fputs( " StandErr_X StandErr_Y    CovarXY\n", fpout);  
#endif
  ...
  while ( (fgets( line, sizeof(line), fpin))  != NULL ) {
	...
	/* distribute threshold evenly into both channels */
	...
	/* calculate estimate of standard error in log_10 ratio */
	
	stderr_lograt = log_ratio_stderr (  muX[internal_index], muY[internal_index],  \
					    stderrX[internal_index], stderrY[internal_index], covXY[internal_index], return_flag, t_x, t_y );

	if ( floor(stderr_lograt)== alargenumber || ceil(stderr_lograt) == alargenumber ) std_err_warning_count++; 

	/* print results */
	fprintf(fpout, "%10.4f  %c %10.4f %10.4f %10.2f %10.4f\n", log_ratio_displayed, return_flag, stderrX[internal_index], stderrY[internal_index], covXY[internal_index], stderr_lograt ) ; 
	

#else 
	/* If mus can be negative, do not print out the log ratios */

	/* print results */
	fprintf(fpout, "%10.4f %10.4f %10.2f\n", stderrX[internal_index], stderrY[internal_index], covXY[internal_index]) ; 
  ...
  if ( std_err_warning_count > 0 ) { 
    printf("Warning: %d log ratio standard error estimates are suspicious\n\n", std_err_warning_count); 
  }
#endif
...
void write_llr_tabbed( FILE *fpin, FILE *fpout, double *llrvals, double *stderrX, double *stderrY, double *covXY ){
  ...
  while ( (fgets( line, sizeof(line), fpin))  != NULL ) {
	...
	internal_index = full2internal[i];
	fprintf(fpout,"\t%.4f\t%.4f", muX[internal_index], muY[internal_index] );
	fprintf(fpout,"\t%.4f", llrvals[internal_index]);
	...
	/* print results */
	fprintf(fpout, "\t%.4f\t%c\t%.4f\t%.4f\t%.4f\t%.4f\n", log_ratio_displayed, return_flag, stderrX[internal_index], stderrY[internal_index], covXY[internal_index], stderr_lograt ) ; 
...
float log_ratio_stderr ( float x, float y, float sig_x, float sig_y, float cov_xy, char return_flag, float xT, float yT ){
  
  float var_lograt, stderr_lograt ; 
  float eps = 1e-3; 
  float xtemp, ytemp; 
  float alargenumber = 9999999.; 

  if ( return_flag != 'T' ) {  

    if ( (x>eps) && (y>eps) ) { 
      var_lograt = pow(1./log(10.),2)*( pow(sig_x/x,2)+ pow(sig_y/y,2) - 2.*cov_xy/x/y ); 
    } else {
      var_lograt = pow(alargenumber,2.) ; // use large numbers as indicators that something is amiss
    }

    if( var_lograt > 0 ){
      stderr_lograt = sqrt( var_lograt ); 
    } else { 
      stderr_lograt = alargenumber; // use large numbers as indicators that something is amiss
    }

  } else { /* if tempered flag use similar idea to tempering approximation for log ratio */

    xtemp = fabs(x) ; 
    ytemp = fabs(y) ; 
    if ( fabs(x) < fabs(xT) ) xtemp = fabs(xT) ; 
    if ( fabs(y) < fabs(yT) ) ytemp = fabs(yT) ; 
    
    /* drop covariance contribution (assume noise not correlated with signal, or noise to noise ) */
    stderr_lograt = sqrt( pow(sig_x/xtemp,2)+ pow(sig_y/ytemp,2) )/log(10.); 

  }

  return stderr_lograt; 
...
float tempered_log_ratio ( float x, float y, float xT, float yT , char *flag ){
  ...
  /* Aug3,200. Special case. Did not merge into below conditions. */
  /* was previously returned as non-tempered */
  if ( x<=1.e-5 && y<=1.e-5 ) {
    *flag = 'T';
    return lr_threshold ; 
  }
  ...
  if ( x >= y && lr_straight > lr_threshold ) {
    ...
    *flag = '-' ; 
    ...
    printf ( "Error: No conditions were met, for x=%f, y=%f, lr_straight=%f, lr_threshold=%f\n",x,y,lr_straight,lr_threshold);
    printf( "Inputs to thresholding function were: x=%f, y=%f, xT=%f, yT=%f\n",x,y,xT,yT);
  }  
